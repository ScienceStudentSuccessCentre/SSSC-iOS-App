...
extension UNUserNotificationCenter {
    ...
    /// - Attention: If the build is running in DEBUG mode (i.e. anything except what's being pushed to the App Store in an archive), a notification will be sent to the user 30 seconds after toggling on the notification for this event.
    ...
    private func determineNotificationDateTime(for event: Event) -> Date? {
        #if DEBUG
        return Calendar.current.date(byAdding: .second, value: 30, to: Date())!
        #else
